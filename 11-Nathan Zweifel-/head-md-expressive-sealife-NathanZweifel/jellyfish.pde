class Jellyfish {
  ...
  void touch(ArrayList<Jellyfish> jellyArray) {
    for (int i = 0; i < jellyArray.size(); i++) {
      if (jellyArray.get(i) != this) {
        if (jellyArray.get(i).posSmooth.x > this.posSmooth.x-(size/2) && jellyArray.get(i).posSmooth.x < this.posSmooth.x + (size/2) && jellyArray.get(i).posSmooth.y > this.posSmooth.y - (size/2) && jellyArray.get(i).posSmooth.y < this.posSmooth.y + (size/2)) {
          color randomColor = color(random(150,255), random(50,120), random(150,255));
          jellyArray.get(i).col = randomColor;
