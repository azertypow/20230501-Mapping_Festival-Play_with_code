class Seaweed {
  ...
  Seaweed(ArrayList<Jellyfish> _jfArray, float _startX, float _startY) {
    this.startX = _startX;
    this.startY = _startY;
    
    //startX = random(0, width);
    //startY = random(0, height);
    //while ((startX > 10 && startX < width-10) && (startY > 10 && startY < height-10)) {
    //  startX = random(0, width);
    //  startY = random(0, height);
    //}
    ...
    this.jf = jfArray.get((int)random(0, jfArray.size()));
  ...
  void display(color colVariation) {
    ...
    //println(x.size());
    //if(x.size() > 400){
    //  this.col = this.transpCol;
    //  //this.transpCol = color(0,0);
    //}
